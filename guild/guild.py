...
class Guild(commands.Cog):
    ...
    async def on_reaction_add(self, react, react_user):
        ...
        if any(USER_ROLE in role.name for role in react_user.roles):
            # TODO FIX "unintened Reaction" from Users without Tyria Role,
            ...
            if self.message:
                if react.message.id == self.message.id:
                    ...
                    if react.count > 1 and react.emoji == "\N{MONEY BAG}":
                        res_mangament = self.bot.get_cog("ResManagement")
                        await res_mangament.print_storage(ctx)
            ...
            if self.confirmation_message:
                if react.message.id == self.confirmation_message.id:
                    if react.emoji == "\N{WHITE HEAVY CHECK MARK}" and react.count == 2:
                        ...
                        await self.print_addon(ctx, self.current_addon)
